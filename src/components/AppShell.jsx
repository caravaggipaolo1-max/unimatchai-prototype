import { BarChart3, BriefcaseBusiness, FileText, Home, LayoutDashboard, LockKeyhole, MessageSquareText, School, ShieldCheck, SlidersHorizontal, Users } from 'lucide-react';
...
const navItems = [
  ...
  { id: 'premium', label: 'Premium center', icon: LockKeyhole, scope: 'Premium' }
];
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          ...
          <div className="hidden items-center gap-2 md:flex">
            ...
            <button onClick={() => setCurrentPage('passport')} className="focus-ring rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-brand-700">Career Passport</button>
          </div>
        </div>
      </header>
              ...
              {navItems.slice(1).map((item) => {
                ...
                return (
                  <button key={item.id} onClick={() => setCurrentPage(item.id)} className={`focus-ring flex min-w-max items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold transition ${active ? 'bg-brand-50 text-brand-700' : 'text-slate-600 hover:bg-slate-50'}`}>
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${item.scope === 'Premium' ? 'bg-brand-900 text-white' : 'bg-slate-100 text-slate-500'}`}>{item.scope}</span>
                  </button>
                );
              })}
